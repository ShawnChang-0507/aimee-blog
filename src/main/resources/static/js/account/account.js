$(function(){
	...
	$(".login").click(function(){
		...
		if ($.trim(name).length < 2 && $.trim(pass).length < 6){
			alert("尊敬的超级无敌炫酷VIP同志，只有输入正确的用户名和密码，萌妹才能帮您办理入村登记哦！O(∩_∩)O~")
			return
		}
		pass = enPs(pass)
	...
	$(".register").click(function(){
		...
		if ($.trim(name).length < 2 || $.trim(name).length > 19){
			alert('亲！用户名字数太少啦~再补两个字呗，请别超过19个哦，村里记事簿只有16开32页，位置有限呢！(*^▽^*)')
			return
		}
		if ($.trim(pass).length < 6 || $.trim(pass).length > 18){
			alert("5个太少，19个太多，你猜密码多少位合适？萌妹会在密码上撒点盐，再打上马赛克~不晓得黑客能不能看清楚，反正萌妹自己已经崩溃了(#^.^#)")
			return
		}
		if (pass != rePass){
			alert('哪个密码才是正确的捏？请保持一致哦 ε=(´ο｀*)))唉')
			return
		}
		pass = enPs(pass)
